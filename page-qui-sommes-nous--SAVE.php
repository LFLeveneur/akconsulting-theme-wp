                ...
                if($query->have_posts()):
                    while($query->have_posts()):
                        ...
                        if (get_the_title() === "About") {
                            echo '<h1>'.get_field("titre_de_la_page").'</h1>';
                        }
                    endwhile;
                endif;
                wp_reset_postdata();
...
<script>
    function dateBar() {
        const heightLastHistoryDate = document.querySelector('.history-date:last-child').offsetHeight;
        const heightHistoryContent = document.querySelector('.history-content').offsetHeight;
        const histotyDateBar = document.querySelector('.history-date-bar');
        histotyDateBar.style.height = heightHistoryContent - heightLastHistoryDate + 'px';
    }
    dateBar();
